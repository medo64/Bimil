...
internal sealed class Twofish : SymmetricAlgorithm {
    ...
    public Twofish()
        ...
        LegalBlockSizesValue = [new KeySizes(minSize: BlockSizeInBits, maxSize: BlockSizeInBits, skipSize: 0)];
        LegalKeySizesValue = [new KeySizes(minSize: 128, maxSize: 256, skipSize: 64)];  // 128, 192, or 256
...
file sealed class TwofishTransform : ICryptoTransform {
    internal TwofishTransform(byte[] rgbKey, byte[]? rgbIV, TwofishTransformMode transformMode, CipherMode cipherMode, PaddingMode paddingMode) {
        ...
        EncryptBlock = GC.AllocateArray<DWord>(BlockSize / 32, pinned: true);
        DecryptBlock = GC.AllocateArray<DWord>(BlockSize / 32, pinned: true);
        DecryptInput = GC.AllocateArray<DWord>(BlockSize / 32, pinned: true);
        ...
        CryptographicOperations.ZeroMemory(MemoryMarshal.AsBytes(key32.AsSpan()));
        ...
        if (rgbIV != null) {
            ...
            CryptographicOperations.ZeroMemory(MemoryMarshal.AsBytes(iv32.AsSpan()));
        }
    ...
    private void Dispose(bool disposing) {
        if (disposing) {
            CryptographicOperations.ZeroMemory(MemoryMarshal.AsBytes(Key.AsSpan()));
            if (IV != null) { CryptographicOperations.ZeroMemory(MemoryMarshal.AsBytes(IV.AsSpan())); }
            CryptographicOperations.ZeroMemory(MemoryMarshal.AsBytes(SBoxKeys.AsSpan()));
            CryptographicOperations.ZeroMemory(MemoryMarshal.AsBytes(SubKeys.AsSpan()));
            CryptographicOperations.ZeroMemory(DecryptionBuffer);
            CryptographicOperations.ZeroMemory(MemoryMarshal.AsBytes(EncryptBlock.AsSpan()));
            CryptographicOperations.ZeroMemory(MemoryMarshal.AsBytes(DecryptBlock.AsSpan()));
            CryptographicOperations.ZeroMemory(MemoryMarshal.AsBytes(DecryptInput.AsSpan()));
        }
    }
    ...
    private readonly DWord[] EncryptBlock;
    private readonly DWord[] DecryptBlock;
    private readonly DWord[] DecryptInput;
    ...
    private static readonly Lock SyncRootBuildMds = new();
